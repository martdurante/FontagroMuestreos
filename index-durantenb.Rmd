...
[APP para clasificar ambientes](https://ee-fontagrobiomasa.projects.earthengine.app/view/clasificacion)  
[Calculadora de Nº de muestras](https://fontagrobiomasa.shinyapps.io/calculadoraNmuestras/)
...
plot_ly(shared_data, y = ~nubes, x = ~fecha, type = 'scatter', mode='lines+markers', line = list(width = 1), color = I(~Localidad), orientation = 'h') %>%
  add_text(text = ~satellite, hoverinfo = "text", showlegend = FALSE) %>% 
  add_trace(y = ~probLluvia, mode = 'lines+markers', line = list(width = 1, dash = 'dot'), showlegend = FALSE) %>%
  add_text(y = ~probLluvia, text = ~satellite, hoverinfo = "text", showlegend = FALSE) %>% 
  layout(title = "Nubes y Probabilidad de lluvia",
         xaxis = list(title = "Fecha"),
         yaxis = list (title = "%"))
...
mediciones %>%
  na.omit()  %>%
  arrange(Pais,Region,Localidad,desc(Fecha)) %>% 
  datatable(filter='top')
```
